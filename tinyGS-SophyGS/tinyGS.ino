...
// Global const
const string str_getMqttServer_Sophy;
...
void wifiConnected()
{
  ...
  std::string str_getMqttServer_Sophy(configManager.getMqttServer_Sophy());
  Log::console(PSTR(str_getMqttServer_Sophy.c_str()));

  if  (str_getMqttServer_Sophy == "localhost") // "192.168.0.17" ) //"localhost")
  {
    Log::console(PSTR("[tinyGS.ino] SOPHY MQTT Server IP not set > local MQTT disabled !"));
  }else{
    Log::console(PSTR("[tinyGS.ino] SOPHY MQTT Server IP set > local MQTT enabled ..."));
    mqtt_fees.begin();
  }
#endif 
...
void loop() {
  ...
  if(Serial.available())
  {
    ...
    switch(serialCmd) {
      ...
      case 'g':
        Log::console(PSTR("toggle output - got it, but not yet implemented !"));
        break;
...
void printControls()
{
  ...
  Log::console(PSTR("g - toggle transmit (tx) signal output"));  
  Log::console(PSTR("------------------------------------"));
}